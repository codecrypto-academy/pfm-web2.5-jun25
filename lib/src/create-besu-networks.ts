...
export interface SignerAccount {
  address: string;
  weiAmount: string;
  minerNode?: string; // Nombre del nodo miner asociado (opcional para compatibilidad)
  _autoGenerated?: boolean; // Internal flag to distinguish auto-generated vs explicit accounts
}
...
export interface BesuNetworkCreateOptions {
  ...
  autoGenerateSignerAccounts?: boolean; // Defaults to true for backward compatibility
}
...
/**
 * Execute command silently, suppressing Docker "not found" error messages
 */
export function executeCommandSilent(command: string): string {
  try {
    return execSync(command, { encoding: "utf-8", stdio: ['pipe', 'pipe', 'pipe'] });
  } catch (error) {
    if (error instanceof Error) {
      // Suppress Docker "not found" error messages
...
export class CryptoLib {
  ...
  publicKeyToAddress(publicKey: string): string {
    ...
    return "0x" + pubKeyBuffer.toString("hex").slice(-40);
  }
...
export class DockerNetworkManager {
  ...
  private handleSubnetConflict(subnet: string): void {
    ...
    try {
      ...
      for (const network of networks) {
        ...
        try {
          const inspect = executeCommandSilent(`docker network inspect ${network}`);
          const networkData = JSON.parse(inspect);
  ...
  networkExists(): boolean {
    try {
      executeCommandSilent(`docker network inspect ${this.networkName}`);
      return true;
    } catch (error) {
      return false;
    }
  ...
  removeNetwork(): void {
    try {
      ...
      try {
        const networkExists = executeCommandSilent(`docker network ls -q --filter "name=^${this.networkName}$"`);
        if (networkExists.trim()) {
          console.log(`🌐 Removing Docker network: ${this.networkName}`);
          executeCommandSilent(`docker network rm ${this.networkName}`);
          console.log(`✅ Network ${this.networkName} removed successfully`);
        } else {
          console.log(`ℹ️  Network ${this.networkName} doesn't exist, nothing to remove`);
        }
      } catch (networkError) {
        // Network might not exist or already removed, that's ok
        const errorMessage = networkError instanceof Error ? networkError.message : String(networkError);
        if (errorMessage.includes('not found') || errorMessage.includes('No such network') || errorMessage.includes('Resource not found')) {
          console.log(`ℹ️  Network ${this.networkName} already removed or doesn't exist`);
        } else {
          console.log(`⚠️  Warning during network removal: ${errorMessage}`);
        }
      }
    } catch (error) {
  ...
  removeContainers(): void {
    try {
      const containers = executeCommandSilent(
        `docker ps -aq --filter "label=network=${this.networkName}"`
      );
      if (containers.trim()) {
        console.log(`📦 Removing containers for network: ${this.networkName}`);
        ...
        try {
          ...
          console.log(`🛑 Stopped containers for network: ${this.networkName}`);
        } catch (stopError) {
          // Some containers might already be stopped
          console.log(`ℹ️  Some containers were already stopped`);
        }
        ...
        console.log(`✅ Removed containers for network: ${this.networkName}`);
      } else {
        console.log(`ℹ️  No containers found for network: ${this.networkName}`);
      }
    } catch (error) {
      // No containers to remove or already removed
      const errorMessage = error instanceof Error ? error.message : String(error);
      if (!errorMessage.includes('Resource not found')) {
        console.log(`ℹ️  Container cleanup completed for network ${this.networkName}: ${errorMessage}`);
      } else {
...
export class BesuNetwork {
  ...
  /**
   * Obtiene las claves de un signerAccount asociado a un miner específico
   */
  getMinerSignerKeys(minerName: string): { privateKey: string; publicKey: string; address: string; enode: string } | undefined {
    return this.minerSignerKeys.get(minerName);
  }

  /**
   * Obtiene todas las claves de signerAccounts asociadas a miners
   */
  getAllMinerSignerKeys(): Map<string, { privateKey: string; publicKey: string; address: string; enode: string }> {
    return new Map(this.minerSignerKeys);
  }

  /**
   * Obtiene información completa de la asociación miner-signerAccount
   */
  getMinerSignerAssociations(): Array<{
    ...
    const result = [];
    const signerAccounts = this.config.signerAccounts || [];
    
    for (const signer of signerAccounts) {
      if (signer.minerNode) {
        const keys = this.minerSignerKeys.get(signer.minerNode);
        if (keys) {
          result.push({
            minerName: signer.minerNode,
  ...
  private validateCreateOptions(errors: ValidationError[], options: BesuNetworkCreateOptions): void {
    ...
    // Validar asociación entre miners y signerAccounts
    this.validateMinerSignerAssociation(errors, options);
  }
  ...
  /**
   * Valida la asociación estricta entre miners y signerAccounts
   */
  /**
   * Automatiza la asociación entre miners y signerAccounts, generando claves internas
   * y asignando automáticamente si no están especificadas.
   */
  private automateMinerSignerAssociation(options: BesuNetworkCreateOptions): void {
    const minerNodes = options.nodes.filter(node => node.type === 'miner');
    let signerAccounts = this.config.signerAccounts || [];
    
    // Si no hay miners, no es necesario hacer nada
    ...
    const unassignedMiners = minerNodes.filter(miner => 
  ...
  private validateMinerSignerAssociation(errors: ValidationError[], options: BesuNetworkCreateOptions): void {
    ...
    // Si no hay signerAccounts configurados, no hay validación que hacer
    if (signerAccounts.length === 0) {
      return;
    }
    
    // Validar que cada miner tenga un signerAccount asociado
    if (minerNodes.length > signerAccounts.length) {
      errors.push({
        field: 'signerAccounts',
        type: 'required',
        message: `Each miner node must have an associated signerAccount. Found ${minerNodes.length} miners but only ${signerAccounts.length} signerAccounts`
      });
    }
    
    // Validar que no haya más signerAccounts que miners
    if (signerAccounts.length > minerNodes.length) {
      errors.push({
        field: 'signerAccounts',
        type: 'invalid',
        message: `Cannot have more signerAccounts than miner nodes. Found ${signerAccounts.length} signerAccounts but only ${minerNodes.length} miners`
      });
    }
    
    // Validar asociaciones específicas si están definidas
    signerAccounts.forEach((signer, index) => {
      if (signer.minerNode) {
        const associatedMiner = minerNodes.find(miner => miner.name === signer.minerNode);
        if (!associatedMiner) {
          errors.push({
            field: `signerAccounts[${index}].minerNode`,
            type: 'invalid',
            message: `SignerAccount ${index} references non-existent miner node '${signer.minerNode}'`
          });
        }
      }
    });
    ...
    const explicitAssociations = signerAccounts
      .map((signer, index) => ({ signer, index }))
      .filter(({ signer }) => signer.minerNode);
      
    const associatedMinerNames = explicitAssociations.map(({ signer }) => signer.minerNode);
    const duplicatedMinerNames = associatedMinerNames.filter((name, index) => 
      associatedMinerNames.indexOf(name) !== index
    );
    
    if (duplicatedMinerNames.length > 0) {
      const uniqueDuplicates = [...new Set(duplicatedMinerNames)];
      errors.push({
        field: 'signerAccounts',
        type: 'duplicate',
        message: `Multiple signerAccounts cannot be associated with the same miner. Duplicated miners: ${uniqueDuplicates.join(', ')}`
      });
    }
  }
  ...
  async create(options: BesuNetworkCreateOptions): Promise<void> {
    // Automatizar asociación de miners con signerAccounts antes de validar (si está habilitado)
    if (options.autoGenerateSignerAccounts !== false) { // Default to true for backward compatibility
      this.automateMinerSignerAssociation(options);
    }
    
    // Validar configuración después de la automatización
    ...
    const minerNodeDefinition = updatedNodes.find(node => node.type === 'miner');
    if (!minerNodeDefinition) {
      throw new Error('No miner node found. At least one miner node is required.');
    }
    
    const minerNode = this.nodes.get(minerNodeDefinition.name);
    if (!minerNode) {
    ...
    // === GUARDAR CLAVES INTERNAS DE SIGNERACCOUNTS ===
    // Guardar las claves privadas de los signerAccounts asociados a miners
    this.saveMinerSignerKeys();
  }
  ...
  async stop(): Promise<void> {
    ...
    // Clean up RPC providers when stopping to prevent hanging connections
    this.destroyRpcProviders();
   ...
   */
  /**
   * Clean up all RPC providers to prevent hanging connections
   */
  private destroyRpcProviders(): void {
    for (const [nodeName, provider] of this.rpcProviders) {
      try {
        // Stop polling if it's active
        if (provider.pollingInterval > 0) {
  ...
  async destroy(): Promise<void> {
    ...
    // Clean up RPC providers first to prevent hanging connections
    this.destroyRpcProviders();
  ...
  async fundMnemonic(
    ...
    const sentTransactions: ethers.TransactionResponse[] = [];
    ...
    // Esperar confirmación de todas las transacciones al final
    if (sentTransactions.length > 0) {
      console.log(`⏳ Waiting for confirmation of ${sentTransactions.length} transactions...`);
      
      // Configurar timeout para evitar esperas indefinidas
      const confirmationTimeout = 60000; // 60 segundos
      ...
      for (let i = 0; i < sentTransactions.length; i++) {
        try {
          ...
          const confirmationPromise = sentTransactions[i].wait();
          ...
          if (error instanceof Error && error.message.includes('timeout')) {
            console.error(`   ⚠️  Transaction ${i + 1} timed out (may still be pending)`);
          } else {
            console.error(`   ⚠️  Transaction ${i + 1} failed to confirm: ${error instanceof Error ? error.message : error}`);
          }
        }
      }
    }
  ...
  getRpcProvider(nodeName: string): ethers.JsonRpcProvider | null {
    ...
    if (this.rpcProviders.has(nodeName)) {
      return this.rpcProviders.get(nodeName)!;
    }
    ...
    // Create provider with shorter timeout and minimal retry
    const provider = new ethers.JsonRpcProvider(rpcUrl, undefined, {
      polling: false, // Disable automatic polling
      staticNetwork: true, // Don't auto-detect network to avoid extra requests
      batchStallTime: 1000, // Reduce batch stall time
      batchMaxSize: 1, // Reduce batch size to minimize hanging requests
      cacheTimeout: 30000 // 30 second cache timeout
    });
    
    // Set shorter timeouts to avoid hanging requests
    provider.pollingInterval = 3000; // 3 seconds
    
    // Cache the provider
    this.rpcProviders.set(nodeName, provider);
  ...
  async getNetworkConnectivity(): Promise<Array<{
    ...
    for (const [nodeName, node] of this.nodes) {
      try {
        ...
        // Set a shorter timeout for connectivity checks
        const timeoutPromise = new Promise((_, reject) => {
        ...
        let peers = 0;
        try {
          // Usando net_peerCount con timeout
          const peerCountPromise = provider.send('net_peerCount', []);
          const peerCountResult = await Promise.race([peerCountPromise, timeoutPromise]) as string;
          peers = parseInt(peerCountResult, 16);
        } catch (e) {
          // Si no funciona, usar admin_peers si está disponible
          try {
            const adminPeersPromise = provider.send('admin_peers', []);
            const adminPeers = await Promise.race([adminPeersPromise, timeoutPromise]) as any[];
  ...
  private generateGenesis(
    minerAddress: string,
    initialBalance: string = "1000000000000000000000000" // Default: 1 million ETH for miner in Wei (valid range)
  ): BesuGenesisConfig {
...
/**
 * Función de conveniencia para crear una red con asociación automática de miners y signerAccounts
 */
export async function createBesuNetworkWithAutoAssociation(
  ...
  // Crear la red con automatización de asociación
  const network = new BesuNetwork(networkConfig, baseDir);
  
  const createOptions: BesuNetworkCreateOptions = {
    nodes,
    initialBalance,
    autoResolveSubnetConflicts
  };
  
  // Crear la red (esto automáticamente asociará miners con signerAccounts)
  await network.create(createOptions);
  
  // Obtener las asociaciones resultantes
  const minerSignerAssociations = network.getMinerSignerAssociations();
  
  // Iniciar la red si se solicita
  if (autoStart) {
    await network.start(besuImage);
    console.log(`🚀 Network ${networkConfig.name} created and started successfully`);
  } else {
    console.log(`📦 Network ${networkConfig.name} created successfully. Use network.start() to run it.`);
  }
