...
const contentToDecipher = fs.readFileSync(inputDataFilePath, {
  encoding: "utf-8",
});
console.log(`Content to decipher: ${contentToDecipher}`);

// cuando el archivo es grande se recomienda usar binary en lugar de hex
const decryptedContent = `${decipher.update(
  contentToDecipher,
  "hex",
  "utf-8"
)}${decipher.final("utf-8")}`; // el resultado es un string utf-8
...
fs.writeFileSync(`${decryptedFilePath}.clean`, decryptedContent, {
  encoding: "utf-8",
});
