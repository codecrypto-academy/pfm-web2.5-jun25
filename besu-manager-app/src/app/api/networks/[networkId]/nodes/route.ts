...
export async function GET(
  ...
  try {
    ...
    for (const container of networkContainers) {
      ...
      // Determinar si es bootnode basándose en el nombre del contenedor
      const isBootnode = container.name.includes('-bootnode-') || container.name.includes('bootnode');
      
      // Solo obtener información detallada si el contenedor está corriendo
      if (container.state === 'running') {
        ...
        // Para contenedores que no están corriendo, usar solo información básica
        console.log(`Container ${container.name} is not running (${container.state}), using basic info only`);
        detailedNodes.push({
          id: container.id,
          name: nodeName,
          status: container.state,
