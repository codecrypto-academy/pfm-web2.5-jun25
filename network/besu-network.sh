...
main() {
    ...
    case "$command" in
        ...
        "start")
            ...
            ;;
        "stop")
            "$SCRIPT_DIR/scripts/stop-network.sh" "${@:2}"
            ;;
        "restart")
            restart_network
            ;;
        "status")
            "$SCRIPT_DIR/scripts/network-utils.sh" status
            ;;
        "logs")
            "$SCRIPT_DIR/scripts/network-utils.sh" logs "${2:-all}"
            ;;
        "test")
            "$SCRIPT_DIR/scripts/test-network.sh"
            ;;
        "reset")
            "$SCRIPT_DIR/scripts/network-utils.sh" reset
            ;;
        "help"|"--help"|"-h")
            show_usage
            ;;
...
setup_complete_network() {
    ...
    # Run all setup steps
    "$SCRIPT_DIR/scripts/setup-network.sh" || {
        log_error "Network setup failed"
        exit 1
    }
    
    "$SCRIPT_DIR/scripts/generate-keys.sh" || {
