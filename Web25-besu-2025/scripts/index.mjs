...
async function getNextworkInfo(url) {
    const version =  await callApi("http://localhost:9090", "net_version", [])
    const peerCount = await callApi("http://localhost:9090", "net_peerCount", [])
    return {
        version,
        peerCount
    };
...
async function main() {
    ...
    switch(command) {
        ...
        case 'network-info':
            const url = args[1] || 'http://localhost:9090';
        ...
        case 'balance':
            ...
            try {
                const balance = await getBalance("http://localhost:9090", balanceAddress);
        ...
        case 'transfer':
            ...
            try {
                const tx = await transferFrom("http://localhost:9090", fromPrivateKey, toAddress, amount);
...
Available commands:
    ...
    network-info [url]   - Get network information (defaults to http://localhost:9090)
